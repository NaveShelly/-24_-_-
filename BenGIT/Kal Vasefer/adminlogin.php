...
<button class="button button2" onclick="parent_login()">כניסת מנהל</button>
</div>
...
<div style="margin: auto;width: 60%;position: absolute; left:250px; top:290px;">
	<div class="form-group">
			<label for="pwd">שם משתמש</label>
			<input type="Child_ID" class="form-control" id="Child_ID_log" placeholder="הקלד כאן" name="Child_ID">
		</div>
		<div class="form-group">
			<label for="pwd">סיסמה</label>
			<input type="password" class="form-control" id="password_log" placeholder="הקלד כאן" name="password">
		</div>
		<input type="button" name="save" class="btn btn-primary" value="התחבר" id="butlogin">
</div>
...
$(document).ready(function() {
	$('#butlogin').on('click', function() {
		...
		if(Child_ID!="" && password!="" ){
			$.ajax({
				...
				success: function(dataResult){
					...
					if(dataResult.statusCode==200){
					    if(Child_ID=="admin"&& password=="admin12345")
					...
					if(dataResult.statusCode!=200)
						...
						$("#error").show();
						$('#error').html('Invalid Id or Password !');
					}
			...
			alert('נא למלא את כל הפרטים המבוקשים');
		}
	});
});
</script>
